...
#pragma NDIS_INIT_FUNCTION(FileBlockerRegisterDevice)


_IRQL_requires_max_(PASSIVE_LEVEL)
NDIS_STATUS
FileBlockerRegisterDevice(
    VOID
    )
{
    ...
    DEBUGP(DL_TRACE, "==>FileBlockerRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = FileBlockerDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = FileBlockerDispatch;
    DispatchTable[IRP_MJ_CLOSE] = FileBlockerDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = FileBlockerDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==FileBlockerRegisterDevice: %x\n", Status);
